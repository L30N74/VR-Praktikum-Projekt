...
public class GestureRecognition_OneHanded : MonoBehaviour {
    ...
    public static void trainingUpdateCallback(double performance, IntPtr ptr)
    {
        ...
        GestureRecognition_OneHanded me = (obj.Target as GestureRecognition_OneHanded);
    ...
    public static void trainingFinishCallback(double performance, IntPtr ptr)
    {
        ...
        {
            me.SaveGesturesFile = "MyOneHandedGestures.dat";
        }
...
/*
    ...
    private List<GameObject> gesturePath;
    private Camera mainCamera;
    [SerializeField] private string loadGesturesFilePath;
    [SerializeField] private string saveGesturesFilePath;
    ...
    private double controller_motion_distance = 0;

    // Timestamp when controller motion was last detected (if pressing the trigger button).
    private System.DateTime controller_motion_time = System.DateTime.Now;
    ...
    private bool record_button_pressed = false;

    public TextMeshProGUID HUDText;
    ...
    void RecordNewGesture() {
        ...
        int throwArms = gr.createGesture("Throw Arms");
        
        Vector3 hmd_position = mainCamera.transform.position;
        Quaternion hmd_quat = mainCamera.transform.rotation;
        gr.startStroke(hmd_position, hmd_quat, throwArms);

        controller.inputDevice.TryGetFeature(CommonUsages.trigger, out float trigger_value);
        while(trigger_value > 0.85) {
            //User is pressing the trigger -> Record gesture
            Vector3 controller_pos = controller.transform.localPosition;
            Quaternion controller_quat = controller.transform.localRotation;
            gr.contdStroke(controller_pos, controller_quat);
        }
