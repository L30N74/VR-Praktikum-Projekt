...
public class GestureRecognition_TwoHanded : MonoBehaviour
{
    ...
    List<GameObject> stroke = new List<GameObject>(); 
    ...
    public GameObject grabbable, grabbable2;
    ...
    void Update()
    {
        ...
        if (trigger_pressed_left == false && trigger_left > 0.9) { 
            ...
            GameObject hmd = mainCamera.gameObject;
        ...
        if (trigger_pressed_left == true)
        {
            ...
            } else
            {
                ...
                float contoller_motion = (left_hand.position - controller_motion_last_left).magnitude;
                controller_motion_last_left = left_hand.position;
        ...
        if (trigger_pressed_right == true)
        {
            ...
            {
                // User still dragging or still moving after trigger pressed
                Transform right_hand = rightController.transform;
                gc.contdStrokeQ(Side_Right, right_hand.position, right_hand.rotation);
                // Show the stroke by instatiating new objects
                addToStrokeTrail(right_hand.position);
                float contoller_motion = (right_hand.position - controller_motion_last_right).magnitude;
                controller_motion_last_right = right_hand.position;
        ...
        foreach (GameObject star in stroke) {
            Destroy(star);
            stroke_index = 0;
        }
        ...
        Renderer grabbableRenderer = grabbable.GetComponent<Renderer>();
        ...
        else if (multigesture_id == 0)
        {
            ...
            grabbable2.GetComponent<Rigidbody>().AddForce(Vector3.forward * 1000 * Time.deltaTime, ForceMode.Impulse);
        }
        else if (multigesture_id == 1)
        {
            HUDText.text = "Identified a heart-shape gesture!";
            grabbable.GetComponent<Rigidbody>().AddForce(Vector3.up * 1000 * Time.deltaTime, ForceMode.Impulse);
        }
        ...
        else if (multigesture_id == 3)
        {
            ...
            grabbableRenderer.material.color = Color.blue;
        }
        else
        {
    ...
    public static void trainingUpdateCallback(double performance, IntPtr ptr)
    {
        ...
        GestureRecognition_TwoHanded me = (obj.Target as GestureRecognition_TwoHanded);
